...
grub_pks_t grub_platform_keystore = { .db = NULL, .dbx = NULL, .db_entries = 0,
                                      .dbx_entries = 0, .use_static_keys = 0 };
                            ...
                            grub_pks_sd_t **pks_sd, grub_size_t *pks_sd_entries)
{
  ...
  rc = grub_sbvar_from_pks (sbvarflags, sbvartype, &esl_data, &esl_data_size);
  /*
      ...
      if (rc == PKS_OBJECT_NOT_FOUND)
        {
           ...
           * So, it loads the Default (static) Keys from ELF Note
           */
